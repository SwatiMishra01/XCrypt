...
# ---------------- Helper Functions ----------------
def sbox_substitute(byte_val):
    row = byte_val >> 4
    col = byte_val & 0x0F
    return SBOX[row][col]

def inv_sbox_substitute(byte_val):
    row = byte_val >> 4
    col = byte_val & 0x0F
...
# Encryption layers
def layer1(state, K1):
    return [(sbox_substitute(b) ^ K1[i % len(K1)]) for i, b in enumerate(state)]
...
def layer1_decrypt(state, K1):
    return [inv_sbox_substitute(b ^ K1[i % len(K1)]) for i, b in enumerate(state)]
